...
class ClientAgreementPDFView(APIView, Common):
    def single_english(self, context, title_style, paragraph_style, bolded_style, underlined_style):
        ...
        main_text = """
        am soliciting assistance in order to enroll in a Health Insurance Plan through the Health Insurance Marketplace. I have provided the necessary information in order to be eligible for the Health Insurance Marketplace tax credit and thus obtain reduced premium costs. I certify that I have received and understand the advice provided by:
        """
        ...
        I hereby give <u><b>{}</b></u> permission to act as my agent, of Health Insurance for my family. By consenting to this agreement, I authorize the use of my confidential information provided in writing, electronically, or by phone, only for one or more of the following purposes:
        ...
        list_items = [
            "Search of and/or creation of an application in the Health Insurance Marketplace.",
            "Complete an application for eligibility and enrollment in a Marketplace Health Plan or other government insurance affordability programs or advance tax credits to help pay Marketplace premiums.",
        ...
        # Create a ListFlowable with ListItem elements
        list_item_style = ParagraphStyle(
            ...
            spaceBefore=0,
            spaceAfter=5,
            bulletFontName='Helvetica',  # Set the font for numbers
            bulletFontSize=8  # Set the font size for numbers
        )
        ordered_list = ListFlowable(
            [ListItem(Paragraph(item, list_item_style), value=i, leftIndent=20)
                for i, item in enumerate(list_items, start=1)],
            bulletType='1',  # Use numbers for the ordered list
    ...
    def double_english(self, context, title_style, paragraph_style, bolded_style, underlined_style):
        ...
        agent_info2 = [
            [Paragraph(f"Secondary Agent: <u><b>{context['secondary_agent_name']}</b></u>", underlined_style),
                Paragraph(
                f"NPN: <u><b>{context['secondary_agent_npn']}</b></u>", underlined_style),
                Paragraph(f"Telephone: <u><b>{context['secondary_agent_tel']}</b></u>", underlined_style)]
        ]
        agent_table2 = Table(agent_info2, colWidths=[
            3.5 * inch, 1.2 * inch, 1.3 * inch])
        agent_table2.setStyle(TableStyle([
            ('ALIGN', (0, 0), (-1, -1), 'LEFT'),
            ('FONTNAME', (0, 0), (-1, -1), 'Helvetica-Bold'),
        ]))
        story.append(agent_table2)

        # Add more text
        story.append(Spacer(5, 2/14 * inch))
        more_text = """

        I hereby give <u><b>{}</b></u> (and) <u><b>{}</b></u> permission to act as my agent,(my agents), of Health Insurance for my family. By consenting to this agreement, I authorize the use of my confidential information provided in writing, electronically, or by phone, only for one or more of the following purposes:
        """.format(context['main_agent_name'], context['secondary_agent_name'])
        more_text_paragraph = Paragraph(more_text, paragraph_style)
        story.append(more_text_paragraph)
    ...
    def double_spanish(self, context, title_style, paragraph_style, bolded_style, underlined_style):
        ...
        story.append(agent_table)
        agent_info2 = [
            [Paragraph(f"Agente Secundario: <u><b>{context['secondary_agent_name']}</b></u>", underlined_style),
                Paragraph(
                f"NPN: <u><b>{context['secondary_agent_npn']}</b></u>", underlined_style),
                Paragraph(f"Teléfono: <u><b>{context['secondary_agent_tel']}</b></u>", underlined_style)]
        ]
        agent_table2 = Table(agent_info2, colWidths=[
            3.5 * inch, 1.2 * inch, 1.3 * inch])
        agent_table2.setStyle(TableStyle([
        ...
        Por este medio doy permiso a <u><b>{}</b></u> (y) <u><b>{}</b></u> para actuar como mi Agente, (mis agentes), de Seguro de Salud de mi familia. Al dar mi consentimiento a este acuerdo, autorizo a utilizar la información confidencial proporcionada por escrito, electrónicamente o por teléfono, solo para los fines de uno o más de los siguientes propósitos:
        """.format(context['main_agent_name'], context['secondary_agent_name'])
        more_text_paragraph = Paragraph(more_text, paragraph_style)
        story.append(more_text_paragraph)
        ...
        list_items = [
            ...
            "Completar una solicitud de elegibilidad e inscripción en un Plan de Salud del Mercado u otros programas gubernamentales de asequibilidad de seguros o créditos fiscales anticipados para ayudar a pagar las primas del Mercado.",
            "Brindar mantenimiento continuo de la cuenta y asistencia para la inscripción según sea necesario.",
            "Responder a consultas del Mercado con respecto a mi solicitud."
        ]
    ...
    def single_spanish(self, context, title_style, paragraph_style, bolded_style, underlined_style):
        ...
        Por este medio doy permiso a <u><b>{}</b></u> para actuar como mi Agente, de Seguro de Salud de mi familia. Al dar mi consentimiento a este acuerdo, autorizo a utilizar la información confidencial proporcionada por escrito, electrónicamente o por teléfono, solo para los fines de uno o más de los siguientes propósitos:
        """.format(context['main_agent_name'])
        more_text_paragraph = Paragraph(more_text, paragraph_style)
        story.append(more_text_paragraph)
    ...
    def post(self, request, format=None):
        ...
        if not signature or not date_signature or not key:
            ...
            if len(client) != 1:
                ...
                data.doc_name.save(
                    f'{consent_log.client_name}_Client_Consent.pdf', File(pdf_buffer))
                pdf_buffer.seek(0)
...
class GetClientLogInformation(APIView):
    def get(self, request: HttpRequest):
        secret_key = request.GET.get('secret_key')
        try:
            client_consent_log = ClientConsentLog.objects.filter(
                secret_key=secret_key).get()
            serializer = GetClientConsentLogSerializer(client_consent_log)
            return Response(serializer.data)
        except:
            raise ValidationException('Log not found')


class SendClientAgreementEmail(APIView):
    ...
    def post(self, request: APIViewRequest):
        ...
        if len(agent) == 1:
            ...
            if lan == 'Es':
                body = f"Tu agente: {agent.agent_name} {agent.agent_lastname}, le ha enviado este Acuerdo de Consentimiento para que lo firme. Puede llamar a su agente para confirmar que esto es oficial. Entre a este link para firmar el Consentimiento: {link}/sign_client_agreement?key={secret_key}"
            else:
                body = f"Your agent: {agent.agent_name} {agent.agent_lastname}, has sended you this Client Consent for you to sign. You can call your agent for confirmation that this is official. Enter to this URL to sign the Client Consent: {link}/sign_client_agreement?key={secret_key}"
...
class SendClientAgreementSMSView(APIView, SmsCommons, Common):
    def generate_random_key(self, length):
        # // 2 to get the appropriate number of bytes
        key = secrets.token_hex(length // 2)
        return key
