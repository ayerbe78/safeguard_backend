...
class IncomeLetterLogViewSet(viewsets.ModelViewSet, AgencyManagement):
    permission_classes = [HasIncomeLetterLogPermission]
    serializer_class = IncomeLetterLogSerializer
    queryset = Agent.objects.all()

    def list(self, request, *args, **kwargs):
        user = request.user
        clients_id = self.get_related_clients(user.pk, True, ['id'])
        year = request.GET.get('year', date.today().year)
        client_logs = IncomeLetterLog.objects.filter(
